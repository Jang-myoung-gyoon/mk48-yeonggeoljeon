...
class CharacterSpriteAssets {
  ...
  static String tile(TerrainType terrain) =>
      'assets/tiles/${terrain.name}.png';
}
...
class OfficerAvatar extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final spriteId = profile.spriteId ?? profile.id;
...
class BattleUnitSprite extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return CharacterSpriteImage(
      assetPath: CharacterSpriteAssets.animation(unit.id, animationState),
      size: size,
      fallback: CharacterSpriteImage(
        assetPath: CharacterSpriteAssets.baseFrame(unit.id),
        size: size,
        fallback: _FallbackAvatar(
          label: unit.name,
